...
def add_like(message_id):
    """User toggle a like for a message for other user's warbles"""
...
def profile():
    ...
    ## need to add something that can remove form label as folder redirection
    if form.validate_on_submit():
        if User.authenticate(g.user.username, form.password.data):
            g.user.username = form.username.data
            g.user.email = form.email.data
            #if form.image_url.data != "/static/images/warbler-hero.jpg" and form.image_url.data == : 
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
                    ...
                    .limit(100)
                    .all())
        ## this if below is to render all messaegs if user isnt following anyone so messages appear
        ## also considered doing a if for Limit to append messages onto page until it reaches 100 limit.
        # this way a new user has interaction on the homepage with other users and messages
